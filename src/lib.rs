...
macro_rules! unroll {
    ...
    ($var:pat_param in ($range:expr).rev() => $body:stmt) => {
        $crate::unroll!($var in ($range).rev().step_by(1) => $body)
    };

    ($var:pat_param in ($range:expr).step_by($step:expr).rev() => $body:stmt) => {
        $crate::unroll!($var in ($range.start..$range.end - ($range.end - $range.start - 1) % $step).rev().step_by($step) => $body)
    };

    ($var:pat_param in $range:expr => $body:stmt) => {
        $crate::unroll!($var in ($range).step_by(1) => $body)
    };
}
