...
const fn available_in_const() {
    ...
    unroll!(_ in (0..25).rev() => {
        a += 1
    });
    unroll!(_ in (0..100).step_by(2) =>
        a += 1
    );
...
#[test]
fn no_underflow() {
    unroll!(_ in (0u64..1).rev() => {});
    let mut iterations: u64 = 0;
    unroll!(_ in (i8::MIN..0).rev() => iterations += 1);
...
fn signed_can_go_negative() {
    ...
    unroll!(i in (-10..11).rev().step_by(5) => actual.push(i));
